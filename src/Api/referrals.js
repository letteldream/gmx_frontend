...
import ReferralStorageVR from "../abis/ReferralStorageVR.json";
...
import { arbitrumReferralsGraphClient, avalancheReferralsGraphClient, rinkebyReferralsGraphClient } from "./common";
import { getContract } from "../Addresses";
import { callContract } from ".";
import { REGEX_VERIFY_BYTES32 } from "../components/Referrals/referralsHelper";

const ACTIVE_CHAINS = [ARBITRUM, AVALANCHE, RINKEBY];
...
function getGraphClient(chainId) {
  if (chainId === ARBITRUM) {
    ...
    return rinkebyReferralsGraphClient;
  }
...
export function useUserCodesOnAllChain(account) {
  ...
  useEffect(() => {
    async function main() {
      const [arbitrumCodes, avalancheCodes, rinkebyCodes] = await Promise.all(
      ...
      const [codeOwnersOnAvax = [], codeOwnersOnArbitrum = [], codeOwnersOnRinkeby = []] = await Promise.all([
        getCodeOwnersData(AVALANCHE, account, arbitrumCodes),
        getCodeOwnersData(ARBITRUM, account, avalancheCodes),
        getCodeOwnersData(RINKEBY, account, rinkebyCodes),
      ]);
      ...
      setData({
        ...
        [RINKEBY]: codeOwnersOnRinkeby.reduce((acc, cv) => {
          acc[cv.code] = cv;
          return acc;
        }, {}),
      });
...
export async function registerReferralCode(chainId, referralCode, library, opts) {
  ...
  const contract = new ethers.Contract(
    ...
    chainId === RINKEBY ? ReferralStorageVR.abi : ReferralStorage.abi,
    library.getSigner()
  );
...
export function useUserReferralCode(library, chainId, account) {
  ...
  const { data: onChainCode } = useSWR(
    ...
    { fetcher: fetcher(library, chainId === RINKEBY ? ReferralStorageVR : ReferralStorage) }
  );
