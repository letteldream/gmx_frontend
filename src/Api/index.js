...
import PositionManagerVR from "../abis/PositionManagerVR.json";
import PositionManager from "../abis/PositionManager.json";
import VaultVR from "../abis/VaultVR.json";
import Vault from "../abis/Vault.json";
import RouterVR from "../abis/RouterVR.json";
import Router from "../abis/Router.json";
import UniPool from "../abis/UniPool.json";
import UniswapV2 from "../abis/UniswapV2.json";
import Token from "../abis/Token.json";
import VaultReaderVR from "../abis/VaultReaderVR.json";
import VaultReader from "../abis/VaultReader.json";
import PositionRouterVR from "../abis/PositionRouterVR.json";
...
import { nissohGraphClient, arbitrumGraphClient, avalancheGraphClient, rinkebyGraphClient } from "./common";
...
export function useInfoTokens(library, chainId, active, tokenBalances, fundingRateInfo, vaultPropsLength) {
  ...
  const { data: vaultTokenInfo } = useSWR(
    ...
    {
      fetcher: fetcher(library, chainId === RINKEBY ? VaultReaderVR : VaultReader, [
        vaultAddress,
...
export function useAllPositions(chainId, library) {
  ...
  const { data: positions = [] } = useSWR(key, async () => {
    ...
    const vaultAddress = getContract(chainId, "Vault");
    const contract = new ethers.Contract(vaultAddress, chainId === RINKEBY ? VaultVR.abi : Vault.abi, provider);
...
export function useMinExecutionFee(library, active, chainId, infoTokens) {
  ...
  const { data: minExecutionFee } = useSWR([active, chainId, positionRouterAddress, "minExecutionFee"], {
    fetcher: fetcher(library, chainId === RINKEBY ? PositionRouterVR : PositionRouter),
  });
...
export function useStakedGmxSupply(library, active) {
  ...
  const gmxAddressRinkeby = getContract(RINKEBY, "GMX");
  const stakedGmxTrackerAddressRinkeby = getContract(RINKEBY, "StakedGmxTracker");

  const { data: rinkebyData, mutate: rinkebyMutate } = useSWR(
    [`StakeV2:stakedGmxSupply:${active}`, RINKEBY, gmxAddressRinkeby, "balanceOf", stakedGmxTrackerAddressRinkeby],
    {
      fetcher: fetcher(undefined, Token),
    }
  );
  ...
  let data;
  if (arbData && avaxData && rinkebyData) {
    data = arbData.add(avaxData).add(rinkebyData);
  }
...
export function useTotalGmxStaked() {
  ...
  const { data: stakedGmxSupplyRinkeby, mutate: updateStakedGmxSupplyRinkeby } = useSWR(
    [
      `StakeV2:stakedGmxSupply:${RINKEBY}`,
      RINKEBY,
      getContract(RINKEBY, "GMX"),
      "balanceOf",
      stakedGmxTrackerAddressRinkeby,
    ],
    {
      fetcher: fetcher(undefined, Token),
    }
  ...
  const mutate = useCallback(() => {
    ...
    updateStakedGmxSupplyRinkeby();
  }, [updateStakedGmxSupplyArbitrum, updateStakedGmxSupplyAvax, updateStakedGmxSupplyRinkeby]);

  if (stakedGmxSupplyArbitrum && stakedGmxSupplyAvax && stakedGmxSupplyRinkeby) {
    let total = bigNumberify(stakedGmxSupplyArbitrum).add(stakedGmxSupplyAvax).add(stakedGmxSupplyRinkeby);
  ...
  return {
    ...
    rinkeby: stakedGmxSupplyRinkeby,
...
export async function approvePlugin(
  ...
  const contract = new ethers.Contract(
    routerAddress,
    chainId === RINKEBY ? RouterVR.abi : Router.abi,
    library.getSigner()
  );
...
export async function _executeOrder(chainId, library, method, account, index, feeReceiver, opts) {
  ...
  const contract = new ethers.Contract(
    positionManagerAddress,
    chainId === RINKEBY ? PositionManagerVR.abi : PositionManager.abi,
    library.getSigner()
  );
  return callContract(chainId, contract, method, params, opts);
}
