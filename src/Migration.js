...
import GmxMigratorVR from "./abis/GmxMigratorVR.json";
...
function MigrationModal(props) {
  ...
  const onClickPrimary = () => {
    ...
    const contract = new ethers.Contract(
      gmxMigratorAddress,
      CHAIN_ID === RINKEBY ? GmxMigratorVR.abi : GmxMigrator.abi,
      library.getSigner()
    );
    contract
...
export default function Migration() {
  ...
  const { data: iouBalances, mutate: updateIouBalances } = useSWR(
    ...
    {
      fetcher: fetcher(library, CHAIN_ID === RINKEBY ? ReaderVR : Reader, [iouTokenAddresses]),
    }
  );
  ...
  const { data: balances, mutate: updateBalances } = useSWR(
    ...
    {
      fetcher: fetcher(library, CHAIN_ID === RINKEBY ? ReaderVR : Reader, [tokenAddresses]),
    }
  );
  ...
  const { data: migratedAmounts, mutate: updateMigratedAmounts } = useSWR(
    ...
    {
      fetcher: fetcher(library, CHAIN_ID === RINKEBY ? GmxMigratorVR : GmxMigrator, [tokenAddresses]),
    }
  );
