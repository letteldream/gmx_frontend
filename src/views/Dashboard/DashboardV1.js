...
export default function DashboardV1() {
  ...
  const { data: pairInfo, mutate: updatePairInfo } = useSWR(
    ...
    {
      fetcher: fetcher(library, chainId === RINKEBY ? ReaderVR : Reader, [
        [gmtAddress, usdgAddress, xgmtAddress, usdgAddress],
      ]),
    }
  );
  ...
  const { data: usdgSupply, mutate: updateUsdgSupply } = useSWR(
    ...
    {
      fetcher: fetcher(library, chainId === RINKEBY ? YieldTokenVR : YieldToken),
    }
  );
  ...
  const { data: fees, mutate: updateFees } = useSWR(
    ...
    {
      fetcher: fetcher(library, chainId === RINKEBY ? ReaderVR : Reader, [whitelistedTokens]),
    }
  );
