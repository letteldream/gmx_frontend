...
import binance16Icon from "../../img/ic_bsc_16.svg";
import ethereum16Icon from "../../img/ic_eth_16.svg";
import arbitrum24Icon from "../../img/ic_arbitrum_24.svg";
import avalanche24Icon from "../../img/ic_avalanche_24.svg";
import binance24Icon from "../../img/ic_bsc_24.svg";
import ethereum24Icon from "../../img/ic_eth_24.svg";

import AssetDropdown from "./AssetDropdown";
import SEO from "../../components/Common/SEO";
import TooltipCard, { TooltipCardRow } from "./TooltipCard";
import useTotalVolume from "../../hooks/useTotalVolume";
const ACTIVE_CHAIN_IDS = [ARBITRUM, AVALANCHE, TESTNET, RINKEBY];
...
export default function DashboardV2() {
  ...
  const { data: fees } = useSWR([`Dashboard:fees:${active}`, chainId, readerAddress, "getFees", vaultAddress], {
    fetcher: fetcher(library, chainId === RINKEBY ? ReaderVR : ReaderV2, [whitelistedTokenAddresses]),
  });

  const { data: totalSupplies } = useSWR(
    [`Dashboard:totalSupplies:${active}`, chainId, readerAddress, "getTokenBalancesWithSupplies", AddressZero],
    {
      fetcher: fetcher(library, chainId === RINKEBY ? ReaderVR : ReaderV2, [tokensForSupplyQuery]),
    }
  );
  ...
  const { data: totalTokenWeights } = useSWR(
    ...
    {
      fetcher: fetcher(library, chainId === RINKEBY ? VaultVR : VaultV2),
    }
  );
  ...
  const totalStatsStartDate =
    chainId === AVALANCHE ? "06 Jan 2022" : chainId === TESTNET ? "26 Jul 2022" : "01 Sep 2021";
  ...
  let glpPool = tokenList.map((token) => {
    const tokenInfo = infoTokens[token.address];
    if (tokenInfo.usdgAmount && adjustedUsdgSupply && !adjustedUsdgSupply.isZero()) {
      const currentWeightBps = tokenInfo.usdgAmount.mul(BASIS_POINTS_DIVISOR).div(adjustedUsdgSupply);
        ...
        <div className="section-title-block">
          ...
          <div className="section-title-content">
            <div className="Page-title">
              ...
              {chainId === RINKEBY && <img src={ethereum24Icon} alt="ethereum24Icon" />}
            </div>
                    ...
                    <TooltipComponent
                      ...
                      renderContent={() => (
                        <TooltipCard
                          ...
                          eth={volumeInfo?.[RINKEBY]?.totalVolume}
              ...
              <div className="App-card">
                <div className="stats-block">
                  <div className="App-card-title">
                    <div className="App-card-title-mark">
                      <div className="App-card-title-mark-icon">
                        ...
                        {chainId === MAINNET && (
                          <img src={binance16Icon} alt="binance16Icon" className="selected-network-symbol" />
                        )}
                        {chainId === TESTNET && (
                          <img src={binance16Icon} alt="binance16Icon" className="selected-network-symbol" />
                        )}
                  ...
                  <div className="App-card-content">
                    ...
                    <div className="App-card-row">
                      <div className="label">Supply</div>
                      <div>{formatAmount(glpSupply, GLP_DECIMALS, 0, true)} DAT</div>
                    </div>
                    <div className="App-card-row">
                      <div className="label">Total Staked</div>
