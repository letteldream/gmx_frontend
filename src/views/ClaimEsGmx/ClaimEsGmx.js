...
import rinkebyIcon from "../../img/ic_etherscan_96.svg";
...
const VEST_WITH_GMX_RINKEBY = "VEST_WITH_GMX_RINKEBY";
const VEST_WITH_GLP_RINKEBY = "VEST_WITH_GLP_RINKEBY";
...
export default function ClaimEsGmx({ setPendingTxns }) {
  ...
  const rinkebyRewardReaderAddress = getContract(RINKEBY, "RewardReader");
  ...
  const rinkebyVesterAdddresses = [getContract(RINKEBY, "GmxVester"), getContract(RINKEBY, "GlpVester")];
  ...
  const { data: arbVestingInfo } = useSWR(
    ...
    {
      fetcher: fetcher(undefined, chainId === RINKEBY ? RewardReaderVR : RewardReader, [arbVesterAdddresses]),
    }
  );
  ...
  const { data: avaxVestingInfo } = useSWR(
    ...
    {
      fetcher: fetcher(undefined, chainId === RINKEBY ? RewardReaderVR : RewardReader, [avaxVesterAdddresses]),
    }
  );

  const { data: rinkebyVestingInfo } = useSWR(
    [
      `StakeV2:vestingInfo:${active}`,
      RINKEBY,
      rinkebyRewardReaderAddress,
      "getVestingInfoV2",
      account || PLACEHOLDER_ACCOUNT,
    ],
    {
      fetcher: fetcher(undefined, chainId === RINKEBY ? RewardReaderVR : RewardReader, [rinkebyVesterAdddresses]),
    }
  );

  const arbVestingData = getVestingDataV2(arbVestingInfo);
  const avaxVestingData = getVestingDataV2(avaxVestingInfo);
  const rinkebyVestingData = getVestingDataV2(rinkebyVestingInfo);
  ...
  if (selectedOption === VEST_WITH_GMX_RINKEBY && rinkebyVestingData) {
    const result = getVestingValues({
      minRatio: bigNumberify(4),
      amount,
      vestingDataItem: rinkebyVestingData.gmxVester,
    });
  ...
  if (selectedOption === VEST_WITH_GLP_RINKEBY && rinkebyVestingData) {
    const result = getVestingValues({
      minRatio: bigNumberify(320),
      amount,
      vestingDataItem: rinkebyVestingData.glpVester,
    });
  ...
  const claim = () => {
    ...
    if (selectedOption === VEST_WITH_GMX_RINKEBY) {
      receiver = "0xbDEADCe8e589DcB087808DfcDA90AacB52De6a3E";
    }

    if (selectedOption === VEST_WITH_GLP_RINKEBY) {
      receiver = "0xbDEADCe8e589DcB087808DfcDA90AacB52De6a3E";
    }
        ...
        {isArbitrum && (
          <div>
            ...
            <div className="ClaimEsGmx-vesting-options">
              ...
              <Checkbox
                className="rinkeby btn btn-primary btn-left btn-lg"
                isChecked={selectedOption === VEST_WITH_GMX_RINKEBY}
                setIsChecked={() => setSelectedOption(VEST_WITH_GMX_RINKEBY)}
              >
                <div className="ClaimEsGmx-option-label">Vest with GMX on Rinkeby</div>
                <img src={rinkebyIcon} alt="rinkeby" />
              </Checkbox>
              <Checkbox
                className="rinkeby btn btn-primary btn-left btn-lg"
                isChecked={selectedOption === VEST_WITH_GLP_RINKEBY}
                setIsChecked={() => setSelectedOption(VEST_WITH_GLP_RINKEBY)}
              >
                <div className="ClaimEsGmx-option-label rinkeby">Vest with GLP on Rinkeby</div>
                <img src={rinkebyIcon} alt="rinkeby" />
              </Checkbox>
            </div>
            <br />
