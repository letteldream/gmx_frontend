...
import YieldTokenVR from "../../abis/YieldTokenVR.json";
import YieldToken from "../../abis/YieldToken.json";
import YieldFarmVR from "../../abis/YieldFarmVR.json";
...
function StakeModal(props) {
  ...
  const onClickPrimary = () => {
    ...
    const contract = new ethers.Contract(
      farmAddress,
      chainId === RINKEBY ? YieldFarmVR.abi : YieldFarm.abi,
      library.getSigner()
    );
    contract
      .stake(amount)
...
export default function StakeV1() {
  ...
  const { data: xgmtSupply, mutate: updateXgmtSupply } = useSWR(
    ...
    {
      fetcher: fetcher(library, chainId === RINKEBY ? ReaderVR : Reader, [XGMT_EXCLUDED_ACCOUNTS]),
    }
  );
  ...
  const { data: balances, mutate: updateBalances } = useSWR(
    ...
    {
      fetcher: fetcher(library, chainId === RINKEBY ? ReaderVR : Reader, [tokens]),
    }
  );
  ...
  const { data: stakingInfo, mutate: updateStakingInfo } = useSWR(
    ...
    {
      fetcher: fetcher(library, chainId === RINKEBY ? ReaderVR : Reader, [yieldTrackers]),
    }
  );
  ...
  const { data: totalStakedInfo, mutate: updateTotalStakedInfo } = useSWR(
    ...
    {
      fetcher: fetcher(library, chainId === RINKEBY ? ReaderVR : Reader, [yieldTokens]),
    }
  );
  ...
  const { data: pairInfo, mutate: updatePairInfo } = useSWR(
    ...
    {
      fetcher: fetcher(library, chainId === RINKEBY ? ReaderVR : Reader, [pairTokens]),
    }
  );
  ...
  const claim = (farmAddress, rewards) => {
    ...
    const contract = new ethers.Contract(
      farmAddress,
      chainId === RINKEBY ? YieldTokenVR.abi : YieldToken.abi,
      library.getSigner()
    );
    contract
      .claim(account)
