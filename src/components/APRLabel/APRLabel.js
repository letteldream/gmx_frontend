...
export default function APRLabel({ chainId, label }) {
  ...
  const { data: walletBalances } = useSWR(
    ...
    {
      fetcher: fetcher(undefined, chainId === RINKEBY ? ReaderVR : ReaderV2, [walletTokens]),
    }
  );
  ...
  const { data: depositBalances } = useSWR(
    ...
    {
      fetcher: fetcher(undefined, chainId === RINKEBY ? RewardReaderVR : RewardReader, [
        depositTokens,
  ...
  const { data: stakingInfo } = useSWR(
    ...
    {
      fetcher: fetcher(undefined, chainId === RINKEBY ? RewardReaderVR : RewardReader, [rewardTrackersForStakingInfo]),
    }
  );
  ...
  const { data: aums } = useSWR([`StakeV2:getAums:${active}`, chainId, glpManagerAddress, "getAums"], {
    fetcher: fetcher(undefined, chainId === RINKEBY ? GlpManagerVR : GlpManager),
  });

  const { data: nativeTokenPrice } = useSWR(
    [`StakeV2:nativeTokenPrice:${active}`, chainId, vaultAddress, "getMinPrice", nativeTokenAddress],
    {
      fetcher: fetcher(undefined, chainId === RINKEBY ? VaultVR : Vault),
    }
  );
