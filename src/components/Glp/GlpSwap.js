...
import RewardTrackerVR from "../../abis/RewardTrackerVR.json";
...
export default function GlpSwap(props) {
  ...
  const { data: tokenBalances } = useSWR(
    ...
    {
      fetcher: fetcher(library, chainId === RINKEBY ? ReaderVR : ReaderV2, [tokenAddresses]),
    }
  );
  ...
  const { data: glpBalance } = useSWR(
    ...
    {
      fetcher: fetcher(library, chainId === RINKEBY ? RewardTrackerVR : RewardTracker),
    }
  );
  ...
  const { data: reservedAmount } = useSWR(
    ...
    {
      fetcher: fetcher(library, chainId === RINKEBY ? VesterVR : Vester),
    }
  );
